<template>
  <el-card style="margin:10px;">
    ...
    <el-table :data="tableData" v-loading="loading" border style="width:100%">
      ...
      <el-table-column prop="type" label="类型标识" width="180" />
      <el-table-column prop="cover_image" label="封面图片" width="120" align="center">
        <template #default="scope">
          <ImagePreview :src="scope.row.cover_image" />
        </template>
      </el-table-column>
      <el-table-column prop="default_prompt" label="默认提示词" min-width="100" show-overflow-tooltip />
      <el-table-column prop="description" label="描述" min-width="260" show-overflow-tooltip />
      <el-table-column label="分类" width="140">
        <template #default="scope">
          <el-tag v-for="cid in scope.row.categoryIds" :key="cid" size="small" style="margin:2px">{{
            getCategoryName(cid) }}</el-tag>
      ...
      <el-table-column label="状态" width="70">
        <template #default="scope">
          <el-tag :type="scope.row.is_active ? 'success' : 'info'" size="small">{{ scope.row.is_active ? '启用' : '禁用'
          }}</el-tag>
        </template>
      </el-table-column>
